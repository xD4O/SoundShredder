# Adapted from Audio-AGI/AudioSep (MIT); see NOTICE.md.
import numpy as np
from typing import Dict, List, NoReturn, Tuple
import torch
import torch.nn as nn
import torch.nn.functional as F
from .transforms import STFT, ISTFT, magphase
...
class ResUNet30_Base(nn.Module, Base):
    ...
    def feature_maps_to_wav(
        ...
        # Y = |Y|cosâˆ Y + j|Y|sinâˆ Y
        #   = |Y|cos(âˆ X + âˆ M) + j|Y|sin(âˆ X + âˆ M)
        #   = |Y|(cosâˆ X cosâˆ M - sinâˆ X sinâˆ M) + j|Y|(sinâˆ X cosâˆ M + cosâˆ X sinâˆ M)
        out_cos = (
    ...
    def forward(self, mixtures, film_dict):
        ...
        # ï¼ˆbatch_size, target_sources_num * output_channels, subbands_num, segment_samples)

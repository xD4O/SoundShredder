...
`npm run test:storage-ui` tests the real folder-picker result path, cancel, application-folder rejection, exclusive-owner rollback, persistence, duplicate instances, narrow layout, disconnected-drive recovery and switching back without losing audio. It uses only private QA folders. Fresh `test:app` runs choose a folder with spaces and verify the real CPU engine, checkpoint and session are created there.
...
**Mac 1.2.2 distribution:** the published Apple Silicon and Intel apps are Developer ID signed and Apple-notarized. Both passed native Gatekeeper, installed-app processing and relaunch checks, and a user confirmed the Finder installation test worked on their Mac. The original 1.2.0 downloads remain unsigned and unchanged. Automatic test packaging uses an ad-hoc identity for integrity testing; only the explicit `developer-id` release workflow establishes Apple trust. See [Mac launch troubleshooting](docs/MACOS.md#finder-says-damaged-or-cannot-be-opened).
...
The current [signed Mac builds passed on both native architectures](https://github.com/xD4O/SoundShredder/actions/runs/35354510031). The separate [Windows 1.2.2 workflow](https://github.com/xD4O/SoundShredder/actions/runs/35315148475) covers its installer and CPU lifecycle. See the [versioned verification record](../VERIFICATION.md) for scope and remaining limitations.
